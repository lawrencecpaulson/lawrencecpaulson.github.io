...
to solve our target problems 
(which were almost certainly drawn from both suggested by Jeremy).
...
We first used [an implementation](https://rdcu.be/c117H) of a procedure due to Hörmander by
McLaughlin and Harrison, with [some success](https://rdcu.be/c118Y).
...
[We published](https://rdcu.be/c119i) our findings using QEPCAD one year later (2008).
...
a canonical form since otherwise the numerous possible variants of any expression
...
two zeroing on proofs in our particular domain.

Most resolution theorem provers are giant incomprehensible programs written in C or C++,
...
by [Joe Leslie-Hurd](https://www.gilith.com).
My team modified Metis more and more over the years, especially with the award
of [a follow-up grant](https://www.cl.cam.ac.uk/~lp15/Grants/AutoPolyFun/), by another postdoc, James Bridge.
James implemented some highly ambitious extensions, 
including [case splitting](https://rdcu.be/bpgqJ),
which was appropriate because of the large numbers of ground literals 
generated by our approach.
We were repeatedly extended the set of special functions that we could support
and the performance.
...
an bound axiom used to replace a function by an upper/lower bound.
...
The generated proofs are typically on the order of hundreds of steps along
